...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            # Actor(id=749, type=vehicle.tesla.model3)
        # Set up the sensors.
    ...
    def render(self, display):
        ...
        for n in range(11):
            ...
            #speed
            blit_v = self.player.get_velocity()
            speed = (3.6*math.sqrt(blit_v.x**2 + blit_v.y**2 + blit_v.z**2)) #3.6
...
class DualControl(object):
    ...
    def _parse_vehicle_wheel(self):
        ...
        throttleCmd = K2 + (2.05 * math.log10(
            -0.4 * jsInputs[self._throttle_idx] + 1.4) - 1.2) / 0.92
        if throttleCmd <= 0:
            throttleCmd = 0
        elif throttleCmd > 1:
            throttleCmd = 1
        ...
        brakeCmd = K3 + (2.05 * math.log10(
            -0.5 * jsInputs[self._brake_idx] + 1.4) - 1.2) / 0.92
        if brakeCmd <= 0:
            brakeCmd = 0
        elif brakeCmd > 1:
            brakeCmd = 1
...
class CameraManager(object):
    def __init__(self, parent_actor, hud):
        ...
        self._camera_transforms = [
            ...
            carla.Transform(carla.Location(x=0.2, y=-0.35, z=1.3), carla.Rotation(pitch=-13)), # x=0.2, y=-0.35, z=1.3
            carla.Transform(carla.Location(x=1.6, y=-0.35, z=1.7), carla.Rotation(pitch=180,roll=180))] # x = 1.6, z = 1.7
